...
for (name in min_max_year_iso3$iso3) {
    for(variable in min_max_year_iso3$var_long){
        year_seq[[name, variable]] <- tibble(
        iso3 = name,
        var_long = variable,
        year = seq(filter(min_max_year_iso3, iso3 == name & var_long == variable)$min - 2,
                   filter(min_max_year_iso3, iso3 == name & var_long == variable)$max + 2,
                   by = 1)
    )
    }
}
