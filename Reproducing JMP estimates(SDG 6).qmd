...
jmpraw %>% 
    count(san_service_chain, var_long, var_short) %>% 
        View()

jmpraw %>% 
    count(var_long, san_service_chain) %>% 
...
jmp_fitted <- jmp_models |>
    ...
    filter(year %in% year_seq_df$year & var_long %in% year_seq_df$var_long & iso3 %in% year_seq_df$iso3)
...
```{r}
#filter(year_seq_df, iso3 == "KEN", var_long == "Septic")$year
...
for (name in unique(year_seq_df$iso3)) {
    
    for (var in filter(year_seq_df, iso3 == name)$var_long) {
        
        for (actyear in filter(year_seq_df, iso3 == name, var_long == var)$year) {
