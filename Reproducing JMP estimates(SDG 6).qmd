...
```{r}
jmp_average_model <- jmp_all_mean_value %>% 
    group_by(iso3) %>% 
    summarise(averageestimate = mean(mean_value))
jmp_average_model
```

```{r}
# Join years data frame with the average of estimates

averageestimates <- left_join(year_seq_df, jmp_average_model, by = "iso3")
averageestimates
...
#if statements for model creation
```{r}

library(broom)

if( (min_max_year_iso3$max - min_max_year_iso3$min) >= 5)  { 
