...
#Prediction using quadratic linear

```{r}


    
uga_rural_improved_quadratic <- lm(mean_value ~ poly(year,3), data = uga_rural_improved)
...
##Adding a quadratic and polynomial term does not change the r squared

```{r}
library(segmented)

yeard <- tibble(year = seq(2000,2020,1))

knot <- 2005 
years.sp <- yeard - knot
years.sp[years.sp < 0] <- 0

plm <- lm(mean_value ~ yeard + years.sp, data = uga_rural_improved)
